...
INPUTPATH = '/home/nly/DNS/adns_depend/input/loop_20240822.txt'
INPUTPATH = '/home/nly/DNS/adns_depend/input/loopindataset_20240822.txt'
# INPUTPATH = '/home/nly/DNS/adns_depend/input/test_3.txt'
...
def save_cache(domain, all_paths):
    """保存缓存到文件"""
            ...
            if not IS_NEXT_TLDADNS: 
                for additional in data['additionals']:
                    if additional['type'] == 'AAAA': #暂时不处理AAAA记录，但是要加到glue_set中，以免authorities里循环请求
                        ...
                        next_ns_vendor = extract_sld(next_ns)
        ...
        if 'authorities' in data and data['authorities']:
            ...
            for authority in data['authorities']:
                if authority['type'] == 'NS':
                    ...
                    # 看看全局cache里有没有这个域名的all_paths，有的话可以直接返回了
                    # 尝试从缓存加载
                    glueless_query_all_paths = load_cache(next_ns)
                    if glueless_query_all_paths:
                        logging.info(f"{next_ns} 命中文件缓存")
                    else:
                        # 需要查询权威服务器的A记录
                        try:
                            dfs_adns_path_alookup(next_ns, ROOT, '192.58.128.30', [], glueless_query_all_paths, current_onpath_domain_ns_list, cache,depth+1)
                            save_cache(next_ns, glueless_query_all_paths) ##############!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                        except RecursionError:
                    ...
                    for glueless_query_path in glueless_query_all_paths:
                        ...
                        if glueless_next_nsip_answers.startswith('$'):
                    ...
                    logging.debug(f"{prefix} {domain} 查询时只获取到SOA {ns} {nsip}\n")
                    new_path = copy.deepcopy(current_path)
...
if __name__ == "__main__":
    ...
    logging.basicConfig(filename=logging_output_file, level=logging.ERROR, format='%(message)s')
    # logging.basicConfig(filename=logging_output_file, level=logging.INFO, format='%(message)s')
