...
def setup_logger(name, log_file, level=logging.WARNING):
    """Function to setup a logger; can be called multiple times to create multiple loggers"""
...
if __name__ == "__main__":
    ...
    for domain in tqdm(domain_list):
        ...
        if not domain.startswith('www.'):
            domain = 'www.' + domain
        DATETIME = (datetime.now() + timedelta(days=OFFSET)).strftime("%Y%m%d-%H%M%S")
        ...
        # logger_name = f"logger_{domain}"
        # logger = setup_logger(logger_name, logging_output_file)
        # 中途输出文件路径
