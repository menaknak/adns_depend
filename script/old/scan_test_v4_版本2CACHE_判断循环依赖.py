...
def run_command(command, max_retries=0, timeout=10):
    """运行系统命令并返回结果，带有重试机制"""
    retry_count = 0 
...
def dfs_adns_path_alookup(domain, ns, nsip, path, all_paths, onpath_domain_ns_list,depth=0):
    """深度优先搜索查询域名A记录路径"""
    global CACHE
        ...
        if 'additionals' in data and data['additionals']:
            for additional in data['additionals']:
                if additional['type'] == 'AAAA': #暂时不处理AAAA记录，但是要加到glue_set中，以免authorities里循环请求
                    ...
                    try:
                        dfs_adns_path_alookup(domain, next_ns, next_nsip, current_path, all_paths, current_onpath_domain_ns_list, depth+1)
                    except RecursionError:
        ...
        if 'authorities' in data and data['authorities']:
            ...
            for authority in data['authorities']:
                if authority['type'] == 'NS':
                    ...
                    try:
                        dfs_adns_path_alookup(next_ns, ROOT, '192.58.128.30', [], glueless_query_all_paths, current_onpath_domain_ns_list, depth+1)
                    except RecursionError:
                    ...
                    for glueless_query_path in glueless_query_all_paths:
                        ...
                        new_path.append(glueless_query_path)
                        try:
                            dfs_adns_path_alookup(domain, next_ns, next_nsip, new_path, all_paths, current_onpath_domain_ns_list, depth+1)
                        except RecursionError:
                            logging.error(f"{prefix}Maximum recursion depth exceeded during authorities processing\n")
...
if __name__ == "__main__":
    ...
    try:
        dfs_adns_path_alookup(domain, ns, nsip, path, all_paths, onpath_domain_ns_list)
    except RecursionError:
