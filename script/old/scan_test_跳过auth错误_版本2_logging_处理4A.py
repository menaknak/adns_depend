...
logging.basicConfig(filename=f"{LOG_DIR}{DATETIME}.txt", level=logging.DEBUG, format='%(message)s')
...
# 文件路径
logging_output_file = f"{OUTPUT_DIR}{DATETIME}_logging.txt"
...
def log_final_path(new_path):
    """将最终路径写入单独的日志文件"""
    with open(logging_output_file, 'a') as log_file:
